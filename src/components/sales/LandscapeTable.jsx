...
export const LandscapeTable = ({ data, columns }) => {
  ...
  const { activeProduct, productsForSale } = useSelector(
    (state) => state.product
  );
  ...
  const selectedIndex = (code, trademark) =>
    productsForSale.findIndex(
      (item) => item.code === code && item.trademark === trademark
    );
      ...
      if (isLoadedProduct) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            prodForSaleSel['total'] =
              Number(prodForSaleSel['qty']) *
              Number(prodForSaleSel['salePrice']);

            const products = replaceItemProdForSale(
              prodForSaleSel,
              productsForSale
            );
        ...
        <thead>
          <tr>
            {Object.keys(data[0]).map(
              (value) =>
                attrib.isCellVisible(value) && (
                  <th key={value}>{attrib.getTitleHeader(value)}</th>
                )
            )}
          </tr>
        </thead>
            ...
            <tr key={key}>
              ...
              {values.hasOwnProperty('trademark') && (
                <ActionButtom
                  type={'select'}
                  row={key}
                  handleClick={handleSelectBtnClick}
                />
              )}
            </tr>
          ))}
